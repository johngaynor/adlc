---
name: pr
description: Use when work is complete and the user wants to open a pull request — "ship it", "open a PR", "put up a PR". This is the ADLC `pr` lifecycle stage: runs validation, commits on the feature branch (creating it if needed), pushes, opens a PR with a structured description, and advances the task to `In Review` when a Linear task resolves.
---
